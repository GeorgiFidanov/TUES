...
struct graph_t{
	...
	int checked;
	struct graph_t **nodes; ///zashtoto e pointer sochest kum pointer
};
...
struct graph_t* init_graph(unsigned int city_number){
    ...
    ///zadelqme pamet za nov graf
	new_graph->city_number = city_number, new_graph->neighbours_count = 0;
	new_graph->checked = 0;
	new_graph->nodes = malloc(sizeof(struct graph_t *));
	///inicializirame nov graf i populvame s podadenite stojnosstti
...
struct graph_t* find_city(struct graph_t* graph, unsigned int city_number){
	...
	if (graph->city_number == city_number)
		return graph;
...
void add_connections(struct graph_t **cities, unsigned int from, unsigned int to){
	...
	to_city->nodes = realloc(to_city->nodes, sizeof(struct graph_t*) * (from_city->neighbours_count + 1));
...
void get_shortest_rout_count(struct graph_t* start, struct graph_t* end, unsigned int *best, int temp){
	if (start == NULL || end == NULL) return; ///ako nachaloto ili kraq e =0
	if (start->checked) return; ///ako sme promenili stojnostta tam da e neshto razlichno ot 0
	if (*best < temp) return; ///ako nqma naj-dobriq put
	...
	if (start == end)
	{
		if (temp < *best){
			*best = temp;
            start->checked = 0;
            return;///proverqvame dali imame naj-dobriq put, ako ne - go slagame
		}else{return;}
	}
	start->checked = 1;
	...
	{
		if (*best <= temp + 1)
			return; //proverqvame dali naj-dobriq ne e po-maluk ili raven tekushtiq
		get_shortest_rout_count(start->nodes[i], end, best, temp+1); ///rekursivno obikalqme vsichki nodove, za da se nameri optimalna distanciq
	}
	start->checked = 0;
}

int has_city(struct graph_t** available_cities, struct graph_t* city_map, unsigned int available_cities_count){
	for (unsigned int i = 0; i < available_cities_count; i++)
		if (available_cities[i]->city_number == city_map->city_number)
			return 1;
	return 0;
}//proverqvame dali daden grad ot kartat  e svoboden
...
void get_cities(struct graph_t** available_cities, struct graph_t* city_map, struct graph_t* home_city, unsigned int *available_cities_count, double cost_between_two_cities, double *available_money){
	...
	unsigned int best = 1 << 31; ///zadavame mu da absurdno golqma stojnost
	get_shortest_rout_count(city_map, home_city, &best, 0);
	if (*available_money - ((best+1) * cost_between_two_cities) < 0) return;

	if (has_city(available_cities, city_map, *available_cities_count) == 0)
	{
...
void main()
{
	...
	struct graph_t** available_cities = (struct graph_t**)malloc(sizeof(struct graph_t*) * city_count);
	if (available_cities == NULL) return;

	unsigned int available_cities_count = 0;
	get_cities(available_cities, city_map, cities[home_city - 1], &available_cities_count, get_cost_between_two_cities(oil_price, car_cost), &available_money);

	for (int i = 0; i < available_cities_count; i++)
		printf("%d\n", available_cities[i]->city_number+1);
