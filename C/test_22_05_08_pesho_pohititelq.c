...
void add_connections(struct graph_t **cities, unsigned int from, unsigned int to)
{
	...
	//struct graph_t**temp_from = (struct graph_t**)malloc(sizeof(struct graph_t*) * from_city->neighbours_count + 1), 
	//	** temp_to = (struct graph_t**)malloc(sizeof(struct graph_t*) * to_city->neighbours_count + 1);

	from_city->nodes = realloc(from_city->nodes, sizeof(struct graph_t*) * (from_city->neighbours_count + 1));
	//for (unsigned int i = 0; i < from_city->neighbours_count; i++)
	//	temp[i] = from_city->nodes[i];
